...
ControllerES9018K2M.prototype.onStop = function() {
  ...
  if (self.i2cBus) {
    try {
      self.i2cBus.closeSync();
      self.logger.info("ControllerES9018K2M::i2c bus closed");
    } catch (err) {
      self.logger.error("ControllerES9018K2M::Error closing i2c bus: " + JSON.stringify(err));
    }
    self.i2cBus = null;
  }
...
ControllerES9018K2M.prototype.getUIConfig = function() {
  ...
  .catch(function(err)
  {
    self.logger.error("ControllerES9018K2M::getUIConfig error: " + JSON.stringify(err));
...
ControllerES9018K2M.prototype.updateUIConfig = function() {
  ...
  var lang_code = self.commandRouter.sharedVars.get('language_code');
  var promise = self.commandRouter.i18nJson(__dirname+'/i18n/strings_' + lang_code + '.json',
      __dirname + '/i18n/strings_en.json',
      __dirname + '/UIConfig.json');
  
  // Handle both kew promises and native promises
  var thenable = promise.then || promise;
  if (typeof thenable !== 'function') {
    self.logger.error("ControllerES9018K2M::updateUIConfig: i18nJson did not return a promise");
    return Promise.resolve();
  }
  
  return Promise.resolve(promise).then(function(uiconf)
  {
    if (!uiconf) {
      self.logger.error("ControllerES9018K2M::updateUIConfig: uiconf is null or undefined");
      return;
    }
  ...
  .catch(function(err)
  {
    self.logger.error("ControllerES9018K2M::updateUIConfig error: " + JSON.stringify(err));
...
ControllerES9018K2M.prototype.initRegister = function()
{
  ...
  // Initialize i2c bus
  try {
    self.i2cBus = i2cBus.openSync(self.i2cBusNumber);
    self.logger.info("ControllerES9018K2M::i2c bus opened successfully");
  } catch (err) {
    self.logger.error("ControllerES9018K2M::Failed to open i2c bus: " + JSON.stringify(err));
    self.i2cBus = null;
  }
};
...
ControllerES9018K2M.prototype.execDeviceCheckControl = function() {
  ...
  if (!self.i2cBus) {
    try {
      self.i2cBus = i2cBus.openSync(self.i2cBusNumber);
      self.logger.info("ControllerES9018K2M::i2c bus reopened successfully");
    } catch (err) {
      self.logger.error("ControllerES9018K2M::Failed to reopen i2c bus: " + JSON.stringify(err));
      ...
      self.updateUIConfig().catch(function(err) {
        self.logger.error("ControllerES9018K2M::execDeviceCheckControl: updateUIConfig error: " + JSON.stringify(err));
      });
  ...
  self.readRegister(self.statusReg).then (function(chipStatus) {
    ...
    if (checkSampleRate) {
      self.parseSampleRate().then (function (sampleRate) {
        ...
        self.logger.error("ControllerES9018K2M::execDeviceCheckControl: parseSampleRate error: " + JSON.stringify(err));
        self.commandRouter.pushToastMessage('info', self.serviceName, message);
      });
    }
    else
...
ControllerES9018K2M.prototype.execResetDeviceControl= function() {
  ...
  self.applyFunction();
  self.updateUIConfig().catch(function(err) {
    self.logger.error("ControllerES9018K2M::execResetDeviceControl: updateUIConfig error: " + JSON.stringify(err));
  });
  self.saveConfig();
...
ControllerES9018K2M.prototype.setBalance = function(value){
  ...
  self.updateUIConfig().catch(function(err) {
    self.logger.error("ControllerES9018K2M::setBalance: updateUIConfig error: " + JSON.stringify(err));
  });
...
ControllerES9018K2M.prototype.execResetBalanceControl = function() {
  ...
  self.config.set('balance', self.balance);
  self.updateUIConfig().catch(function(err) {
    self.logger.error("ControllerES9018K2M::execResetBalanceControl: updateUIConfig error: " + JSON.stringify(err));
  });
};
...
ControllerES9018K2M.prototype.getSampleRate = function() {
  ...
  return self.readRegister(66).then(function(value1) {
    reg66 = value1;
    return self.readRegister(67);
  }).then(function(value2) {
    reg67 = value2;
    return self.readRegister(68);
  }).then(function(value3) {
    reg68 = value3;
    return self.readRegister(69);
...
ControllerES9018K2M.prototype.parseSampleRate = function() {
  ...
  return self.getSampleRate().then(function (sampleRate) {
    if (sampleRate) {
...
ControllerES9018K2M.prototype.readRegister = function(regAddr) {
  ...
  if (!self.i2cBus) {
    self.logger.error("ControllerES9018K2M::readRegister: i2c bus not initialized");
    return Promise.resolve(null);
  }

  try {
    var result = self.i2cBus.readByteSync(self.deviceAddress, regAddr);
...
ControllerES9018K2M.prototype.writeRegister = function(regAddr, regVal) {
  ...
  if (!self.i2cBus) {
    self.logger.error("ControllerES9018K2M::writeRegister: i2c bus not initialized");
    return;
  }

  try {
    self.i2cBus.writeByteSync(self.deviceAddress, regAddr, regVal);
  }
  catch (err) {
